...
function GET (req, res) {
  ...
  if(id != 0) {
    new Model({IDEN_USUARIO: id}).fetch({withRelated: ['telefonos', 'persona', 'emprendedor', 'emprendedor.imagen']})
      .then(entity => {
        if(!entity) {
    ...
    new Collection().fetch({withRelated: ['telefonos', 'persona', 'emprendedor']})
      .then(entities => {
